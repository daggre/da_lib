...
local RemoveProp = function(data)
    ...
    if not removedData then
        log.spam(("Not removing prop: %s"):format(data.id))
        return
    end
    log.spam("Removing:", data.id, removedData)
...
RegisterBlockingServerEvent("intprop:server:remove", function(source, data)
...
RegisterBlockingServerEvent("intprop:server:updateAmount", function(source, propData, amount)
    ...
    if cachePropData and cachePropData.metadata and cachePropData.metadata.resourceAmount then
        ...
        log.debug(("Changed resource amount %.3f to %.3f"):format(amount, cachePropData.metadata.resourceAmount))
