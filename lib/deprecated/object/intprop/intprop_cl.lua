...
local CreateProp = function(data)
    ...
    log.debug("Creating intprop:", data)
    -- Create the object
    local entity = da_obj.create(data.objectHash, data.coords, data.spawnParams)
    ...
    if data.objectHash == `p_bucket03x` and data.metadata and data.metadata.resourceAmount then
        ...
        da_obj.expression(entity, "bucket_Fill", data.metadata.resourceAmount)
        log.spam("Creating bucket, setting resource amount to "..tostring(data.metadata.resourceAmount))
    end
    -- Cache the object
...
local AddInteractivePropZone = function(data)
    ...
    log.debug("Adding IntProp InteractivePropZone", data)
...
local RemoveProp = function(data)
    ...
    log.debug("Removing IntProp Prop Zone", data)
    -- Delete the prop
    DeleteProp(data.id)
...
local AddProp = function(propData)
    assert(type(propData.objectHash) == "number", string.format("Invalid prop: %s", log.format(propData.objectHash)))
    assert(type(propData.coords) == "vector3", string.format("Invalid coords: %s", log.format(propData.coords)))
...
local function _checkPropData(objectHash, coords, metadata)
    ...
    assert(type(coords) == "vector3", string.format("Invalid coords: %s", log.format(coords)))
    assert(type(metadata) == "table", string.format("Invalid metadata: %s", log.format(metadata)))
    ...
    -- assert(type(propData.metadata.resourceAmount) == "number", string.format("Invalid resourceAmount: %s", log.format(propData.metadata.resourceAmount)))
end
...
Lib.Obj.CreateIntProp = function(ojbectHash, coords, metadata, spawnParams)
    ...
    if spawnParams and spawnParams.ground then
        ...
        local entity = da_obj.create(ojbectHash, coords, spawnParams)
        if entity then
