...
local function LoadScene(data)
    ...
    for _, prop in ipairs(data.Props) do
        ...
        if prop.vehicle then
            obj = da_obj.createVehicle(prop.model, prop.position, prop)
        else
            obj = da_obj.create(prop.model, prop.position, prop)
        end
        ...
        if obj and LoadedScenes[data.name] then
            ...
            if prop.animation then
                log.spam(("Playing animation %s on %s"):format(prop.animation.anim, prop.model))
                da_anim.object(obj,
                    prop.animation.dict,
                    prop.animation.anim,
                ...
                if prop.animation.flags then
                    ...
                    if animSpeed ~= nil or animTime ~= nil then
                        ...
                        SetTimeout(200, function()
                            log.debug(("Setting object %s time:%s speed:%s"):format(obj, animTime or "-", animSpeed or "-"))
                            da_anim.set(obj, prop.animation.dict, prop.animation.anim, animTime, animSpeed)
                        end)
                    end
                end
            end
    ...
    if (objectGroup or data.GroupEvent) and LoadedScenes[data.name] then
        if not objectGroup then log.warn("GroupEvent is set but no objectGroups created"); end
        local groupEvent = data.GroupEvent or ("%s:ObjectGroup"):format(data.name)
        -- Callback to deliver entity groups
        log.debug("Triggering group event", groupEvent, objectGroup)
...
Lib.Props.Register = function(sceneName, scene)
    ...
    if not scene.Zone or not scene.Zone.Radius or not scene.Zone.Coords then
        log.warn(scene.Zone.Radius, ("Zone malformed %s"):format(sceneName))
        return
    end
    ...
    if LoadedScenes[sceneName] then
        ...
        while LoadedScenes[sceneName] and LoadedScenes[sceneName].loaded == false and GetGameTimer() < unloadTimeout do
            ...
            if GetGameTimer() >= unloadTimeout then
                log.debug(("Failed load scene, it never finished unloading '%s'"):format(sceneName))
                return
            end
        end
